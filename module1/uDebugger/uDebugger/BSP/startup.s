...
HardFaultIrqHandler
      ...
      //LR contains exception "code" value 0xFFFF_FFF9 which tells the 
      //processor to handle the return differently
      //<your code here>
      PUSH {R14}
      
      //Copy the PC and LR values from the stack to R0 and R1 respectively.
      //notes: PC and LR are in stack which is stored in SRAM. Stack pointer (R13)
      //contains the top of the stack. LR and PC can be accessed using SP with
      //an offset
      
      //Copy SP to R0
      //use R0 as scratch pad register to add offsets
      //<your code here>
      MOV R0,R13
      
      //Add offset to R0 so it points to location where PC was pushed on stack
      //R0 now points to Program Counter (instruction that caused the exception to occur)
      //<your code here>
      ADD R0,R0,#28
      
      //Save R0 to stack since we will want to reference it later
      //R0 is now on top of STACK
      //<your code here>
      PUSH {R0}
      ...
      //R1 now poinst to the link register
      //<your code here>
      SUB R1,R1,#4
